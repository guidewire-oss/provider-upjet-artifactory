		...
		It("should exist in Artifactory read instance", func(ctx SpecContext) {
			...
			DeferCleanup(func(ctx SpecContext) {
				...
				Eventually(func() bool {
					...
					err := k8sClient.Get(ctx, client.ObjectKey{Name: "test-local-npm-repo"}, repo)
			...
			err = rtWriteClient.GetRepository("test-local-npm-repo", &repoDetails)
			Expect(err).NotTo(HaveOccurred())
			Expect(repoDetails.Key).To(Equal("test-local-npm-repo"))
